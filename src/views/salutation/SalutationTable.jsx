...
const SalutationTable = () => {
  ...
  const handleSwitchChange = index => async event => {
    const updatedRows = [...customerDataList]
    updatedRows[index].n_status = event.target.checked ? 1 : 0
    setCustomerDataList(updatedRows)
    ...
    <Box>
      <Box display={'flex'} justifyContent={'space-between'} mb={4}>
        ...
        />
          ...
          <Button  onClick={() => setOpen(true)} startIcon={<i className='ri-add-line'></i>} variant='contained' className='mis-4'>
            Add Salutation
          </Button>
            ...
            <Paper sx={{ width: '100%', overflow: 'hidden' }}>
              <TableContainer sx={{ maxHeight: 440 }}>
                <Table stickyHeader aria-label='sticky table'>
                  ...
                  <TableBody>
                    {customerDataList.map((row, index) => (
                      <TableRow hover role='checkbox' tabIndex={-1} key={row._id}>
                        <TableCell sx={{ width: '60px' }}>{index + 1}</TableCell>
                        <TableCell>
                          {' '}
                          <Link href={`/leads/edit-lead/${row?.salutation_name}`}>
                            {' '}
                            <b>{row?.salutation_name}</b>
                          </Link>{' '}
                        </TableCell>
