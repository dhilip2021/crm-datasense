    ...
    if (emailField && emailField.value.trim() !== "") {
      ...
      if (!emailCheck) {
        ...
        setErrors((prevErrors) =>
          prevErrors.includes("email") ? prevErrors : [...prevErrors, "email"]
        );
      } else {
    ...
    if (name === "email") {
      setInputs((prevInputs) =>
        prevInputs.map((input) =>
          input.slug_label === name
            ? { ...input, value: normalizeEmail(value) }
            : input
        )
      );
      ...
      if (value === "" || !isNaN(res)) {
        const formattedValue =
          value === "" ? "" : formatIndianNumber(cleanedValue);
      ...
      if (value === "" || res) {
        setInputs((prevInputs) =>
          prevInputs.map((input) =>
            input.slug_label === name ? { ...input, value: value } : input
          )
        );
      ...
      setInputs((prevInputs) =>
        prevInputs.map((input) =>
          input.slug_label === name
            ? { ...input, value: normalizeEmail(formatURL(value)) }
            : input
        )
      );
      if (normalizeEmail(value)) removeError(name);
