...
export async function POST(req) {
  ...
  try {
    ...
    const lastEntry = await Customerform.findOne().sort({ _id: -1 }).lean()
    ...
    const customerformData = new Customerform({
      organization_id,
      auto_inc_id: nextAutoId,
      // lead_id: create_UUID(),
      customer_id: lead_slug_id,
      customer_name,
      customer_slug_name,
      form_name,
      values: {
    ...
    await customerformData.save()

    return NextResponse.json({
      success: true,
      message: 'Customer form saved',
      data: {
        organization_id,
        auto_inc_id: nextAutoId,
        customer_name,
        customer_slug_name,
        form_name,
        values: customerformData.values,
        submittedAt: customerformData.submittedAt,
      }
    })
  } catch (error) {
