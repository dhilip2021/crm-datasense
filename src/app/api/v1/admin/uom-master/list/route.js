...
export async function GET(request) {
  ...
  if (verified.success) {
    ...
    if (id) {
      const uomId = await UOMMaster.findOne({ uom_id: id })

      if (uomId) {
        let _search = {}
        _search['$and'] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { uom_id: id }, { organization_id: organizationId }]
          }
        ]

        try {
          await connectMongoDB()
          await UOMMaster.aggregate([
            { $match: _search },
            {
              $group: {
                _id: '$_id',
                uom_code: { $first: '$uom_code' },
                uom_label: { $first: '$uom_label' },                
                uom_id: { $first: '$uom_id' },
...
export async function POST(request) {
  ...
  if (verified.success) {
    ...
    if (searchTerm !== '') {
      ...
      //     uom_code: { $regex: searchTerm, $options: 'i' }
      //   }
      let _search = {
        n_published: 1,
        organization_id: organizationId,
        $expr: { $regexMatch: { input: { $toString: '$uom_code',$toString: '$uom_label' }, regex: searchTerm, options: 'i' } }
      }

      try {
        await connectMongoDB()

        const data = await UOMMaster.aggregate([
          { $match: _search },
          {
            $group: {
              _id: '$_id',
