    ...
    if (verified.success) {
      ...
      if (searchTerm !== '') {
        if (verified.data.c_role_id === '16f01165898b') {
          _search['$and'] = [
            {
              $and: [{ c_privilege_name: { $regex: searchTerm, $options: 'i' } }, { n_status: 1 }, { n_published: 1 }]
            }
          ]
        } else {
          _search['$and'] = [
...
export async function GET(request) {
  ...
  const roleId = request.nextUrl.searchParams.get('roll')
    ...
    if (id) {
      ...
      if (checkId) {
        ...
        if (verified.data.c_role_id === '16f01165898b') {
          _search['$and'] = [
            {
              $and: [{ n_status: 1 }, { n_published: 1 }, { c_privilege_id: id }]
            }
          ]
        } else {
          _search['$and'] = [
      ...
      const checkId = await UserPrivileges.findOne({ c_role_id: roleId })
      if (checkId) {
        let _search = {}
        if (verified.data.c_role_id === '16f01165898b') {
          _search['$and'] = [
            {
              $and: [{ n_status: 1 }, { n_published: 1 }, { c_role_id: roleId }]
            }
          ]
        } else {
          _search['$and'] = [
        ...
        try {
          ...
          // now group
          {
            $group: {
              ...
              c_privilege_id: { $first: '$c_privilege_id' },
              c_role_id: { $first: '$c_role_id' },
              c_role_name: { $first: '$userroles.c_role_name' }, // ✅ correct place
              c_role_priority: { $first: '$userroles.c_role_priority' }, // ✅ correct place
              c_role_privileges: { $first: '$c_role_privileges' },
              c_menu_list: { $first: '$c_menu_list' },
              c_menu_privileges: { $first: '$c_menu_privileges' },
              n_status: { $first: '$n_status' },
...

          {
            $project: {
              ...
              c_privilege_id: 1,
              c_role_id: 1,
              c_role_name: 1, // 👌 now this will appear
              c_role_priority: 1,
              c_role_privileges: 1,
              c_menu_list: 1,
              c_menu_privileges: 1,
