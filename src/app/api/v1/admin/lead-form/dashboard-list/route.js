...
export async function GET(req) {
  ...
  try {
    ...
    // 🟢 If page & limit are NOT provided → get all data (no pagination)
    let data, total
    if (!page || !limit) {
      data = await Leadform.find(query).sort({ createdAt: -1 }).select('-__v').lean()
      total = data.length
    } else {
      ...
      data = await Leadform.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .select('-__v')
      ...
      total = await Leadform.countDocuments(query)
    }

    const statsResult = await Leadform.aggregate([
    ...
    return NextResponse.json({
      ...
      total,
      totalPages: page && limit ? Math.ceil(total / parseInt(limit, 10)) : null,
      stats:
        statsResult[0] || {
          totalLeads: 0,
          hotLeads: 0,
