  ...
  return (
    <>
      {leadData?.items?.map(order => {
        ...
        order.item_ref
          .filter(p => p.itemMasterRef?.item_type === 'Service')
          .forEach(p => {
            const itemQty = p.quantity || 0
            const serviceCharge = p.service_charge || 0
            const itemSubtotal = itemQty * serviceCharge
        ...
        return (
          <>
            {order.item_ref.some(p => p?.itemMasterRef?.item_type === 'Service') && (
              <Paper
                key={order._id}
                ...
                <Table size='small'>
                  <TableHead>
                    <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                      {['Service Name', 'Item Type', 'Service Charge', 'UOM', 'duration', 'type', 'GST'].map(head => (
                        <TableCell
                          key={head}
                          sx={{ fontWeight: 700, fontSize: 13, textTransform: 'uppercase', color: '#555' }}
                          align={
                            ['service_charge', 'service_duration', 'service_type'].includes(head) ? 'right' : 'left'
                          }
                        >
                          {head}
                        </TableCell>
                  ...
                  <TableBody>
                    {order.item_ref
                      .filter(p => p?.itemMasterRef?.item_type === 'Service') // 🟢 Only Service Items
                      .map((p, idx) => (
                        <TableRow key={p._id || idx}>
                          <TableCell sx={{ py: 2, fontSize: 13 }}>{p.service_name || '—'}</TableCell>
                          <TableCell>
                            <Chip label={p.itemMasterRef?.item_type || '—'} size='small' color='warning' />
                          </TableCell>
                          <TableCell align='center'>{formatCurrency(p.service_charge || 0)}</TableCell>
                          <TableCell align='center'>{p.uom || '—'}</TableCell>
                          <TableCell align='right'>{p.service_duration || '—'}</TableCell>
                          <TableCell align='right'>{p.service_type || '—'}</TableCell>
